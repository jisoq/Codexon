...
PR은 선택 사항이며 `main`에 직접 push할 수 있습니다. 배포할 때는 새 `VERSION`을 반영한 뒤 `gh workflow run windows.yml --ref main -f publish=true`를 실행합니다. 전체 검증을 통과한 패키지로 운영 설치기와 체크섬·서드파티 소스를 만들고, 같은 실행의 산출물을 별도 게시 작업에 전달합니다. 게시 작업만 릴리즈 쓰기 권한을 가지며, 모든 자산을 초안에 업로드한 뒤 공개합니다. 태그 `v<VERSION>`은 실행 시작 시 선택된 커밋을 가리키고 기존 태그는 덮어쓰지 않습니다. `publish`를 선택하지 않은 수동 실행은 전체 검증만 합니다. 브랜치의 PR 필수·push 전 CI 통과 조건 대신 게시 작업의 CI 성공 의존성으로 배포를 제어하며, 강제 push·브랜치 삭제 금지는 유지합니다.
