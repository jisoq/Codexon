"""Theme selection propagates to both windows without losing user state."""
...
from cachemonitor.token_colors import TOKEN_COLORS
...
def test_quick_theme_selection_updates_overlay_and_preserves_state(tmp_path):
    ...
    try:
        ...
        for index, mode in ((2, 'dark'), (1, 'light'), (0, 'codex')):
            click(window, item)
            popup = item.findChild(QObject, 'choice-popup')
            assert popup.property('visible')
            label = choice.itemText(index)
            option = next(child for child in walk(popup.property('contentItem'))
                          if child.property('text') == label and child.isVisible())
            click(window, option)
            refresh()
