...
For a request to install Codexon or prepare a usable local environment, follow [INSTALL.md](INSTALL.md). For ordinary use, ask the user to run the release Setup themselves and wait for their installation-complete message before verifying installation, environment and settings. Do not execute Setup on their behalf. Do not offer a portable ZIP; retain source setup for development and safe migration of legacy portable copies. Complete runtime and actual interface checks before reporting readiness.
...
- Release Windows x64 `Codexon-Setup.exe` and `Codexon-Setup.exe.sha256`, and verify the remote asset digest. The checksum is for automatic app updates; ordinary users only download and run Setup. Do not publish a portable app ZIP. Public screenshots and logs must use synthetic or redacted data.
