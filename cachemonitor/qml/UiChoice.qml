...
ComboBox {
    ...
    contentItem: Text {
        ...
        color: control.enabled ? (appTheme.palette && appTheme.color("#243247")) : (appTheme.palette && appTheme.color("#98a2b3"))
    ...
    delegate: ItemDelegate {
        ...
        contentItem: Text {
            ...
            font: control.font; color: (appTheme.palette && appTheme.color("#243247"))
            wrapMode: Text.Wrap
            verticalAlignment: Text.AlignVCenter
        }
        UiToolTip { visible: option.hovered && text.length > 0; text: modelData.tooltip || "" }
        background: Rectangle { Rectangle { anchors.fill:parent;radius:4;color:appTheme.palette.accent;opacity:option.hovered ? .09 : 0 } color: highlighted ? (appTheme.palette && appTheme.color("#edf4fc")) : (appTheme.palette && appTheme.color("white")); radius: 4 }
    }
    popup: Popup {
        ...
        background: Rectangle { color: (appTheme.palette && appTheme.color("white")); radius: 6; border.color: (appTheme.palette && appTheme.color("#cdd5df")) }
    }
}
