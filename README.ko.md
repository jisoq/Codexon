...
**설정 → 캐시 관리**를 켜면 전용 탭에서 관측 상태, 유지 판단, 예상·관측 환산액과 최근 활동을 확인할 수 있습니다. 자동 유지와 모델 변경 확인은 각각 선택하며 기본적으로 꺼져 있습니다. 독립 유지 요청에는 별도의 운용 동의가 필요합니다. 예상 이득과 실제 절감은 구분하며, 확인되지 않은 사용량을 0으로 계산하지 않습니다. 연결과 허용 범위는 [캐시 관리 안내](docs/user-guide.ko.md#캐시-관리)를 참고하세요.
